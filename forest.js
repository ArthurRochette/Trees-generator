...
function branches(last_pos, group = [], generation = 0) {
    ...
    var angle = (PI / 2) / nbr_ramot;
    angle += PI * Math.floor(Math.random() * 2)
    for (var i = 0; i < nbr_ramot; i++) {
        const points = []
        points.push(last_pos)
        points.push(new THREE.Vector3(
            last_pos.x + Math.random() * LARGEUR_FEUILLAGE*4 - LARGEUR_FEUILLAGE*2,
            last_pos.y + BRANCH_SIZE * Math.cos(angle * i) * HAUTEUR_FEUILLAGE,
